...
int reshape_tree() {
    ...
    const char* inDir = "/gpfs/ddn/srm/cms/store/user/calexe/SingleMuon/CVH_refit_Data/250305_112633/0000/";
    const char* ext = ".root";
    TString outDir = "./inoutfiles/SingleMuon/CVH_refit_Data/";
    ...
    while(((entry) = (char*)gSystem->GetDirEntry(dirp))) {
        TString str = entry;
        if(str.EndsWith(ext)) {
            const char* filename = gSystem->ConcatFileName(dir, entry);
            ...
            str.ReplaceAll(ext, "_reshaped.root");
            TFile* fout = TFile::Open(outDir+str, "RECREATE");
